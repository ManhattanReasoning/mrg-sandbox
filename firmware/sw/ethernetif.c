...
/*
 * Per-FPGA locally-administered MAC.  FPGA_ID (0-9) is supplied at build time
 * (-DFPGA_ID=, default 0) so each board gets a distinct address; the host's
 * config.py uses the same formula: 02:00:00:00:00:(01 + fpga_id).
 */
#ifndef FPGA_ID
#define FPGA_ID 0
#endif

static const uint8_t fpga_mac[6] = {0x02, 0x00, 0x00, 0x00, 0x00,
                                    0x01 + FPGA_ID};
